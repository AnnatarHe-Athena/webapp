    ...
    return (
      <tr key={i}>
        {Object.keys(x).map((k, ind) => {
          ...
          if (k === 'cate') {
            return <td key={ind}>{categories.find(item => item.id === (x as any)[k])?.name}</td>
          }
          if (k === 'permission') {
            return <td key={ind}>{premissionOptions.find(item => item.value === (x as any)[k])?.label}</td>
          }
    ...
    <PageContainer>
      <Card>
        ...
        <ReadyToUpload>
          <thead>
            <tr>{
              ['图片URL', '图片介绍', '分类', '权限', 'fromID', 'fromURL'].map((x, i) => {
        ...
        <form onSubmit={e => {
          ...
          <InputField>
            <input type="url" name='img' onChange={formikData.handleChange} placeholder="URL" value={formikData.values.img} />
            <input type="text" name='text' onChange={formikData.handleChange} placeholder="TEXT" value={formikData.values.text} />
            <input type="text" name='fromID' onChange={formikData.handleChange} placeholder="FROM ID" value={formikData.values.fromID} />
            <input type="text" name='fromURL' onChange={formikData.handleChange} placeholder="FROM URL" value={formikData.values.fromURL} />
            <Select
              name="cate"
